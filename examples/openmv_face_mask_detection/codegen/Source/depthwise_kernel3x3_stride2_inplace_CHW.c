...
void depthwise_kernel3x3_stride2_inplace_kernel_CHW(const uint16_t output_y, const uint16_t output_x,
...
tinyengine_status depthwise_kernel3x3_stride2_inplace_CHW(
    ...
    for (c = 0; c < input_ch; c++) {
        ...
        depthwise_kernel3x3_stride2_inplace_kernel_CHW(
            output_y, output_x, bias++, biasR++, ksrc, output_mult++, output_shift++, inplace_out, output_offset,
            output_activation_min, output_activation_max, cols_8b_start, input_x, input_ch);
        ksrc += 9;
        input++;
    }
}
