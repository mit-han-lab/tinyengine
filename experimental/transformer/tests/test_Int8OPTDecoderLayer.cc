...
void test_DecoderLayer_generate() {
    ...
    read_to_array("assets/tests/generate_layer0_attention_mask.bin", attention_mask.m_data, b * sqlen * sqlen);
    ...
    read_to_array("assets/tests/generate_layer0_residual.bin", residualGT.m_data, b * sqlen * embed_dim);
...
void test_DecoderLayer_generate_cache() {
    ...
    // print_first_k_elelment("input.attention_mask.m_data", input.attention_mask.m_data, 10);
    // print_first_k_elelment("input.past_key.m_data", input.past_key.m_data, 10);
    struct Int8OPTDecoderLayer_output output = layer.forward(input);
    // print_first_k_elelment("output.past_key_value.first.m_data,", output.past_key_value.first.m_data, 10);
