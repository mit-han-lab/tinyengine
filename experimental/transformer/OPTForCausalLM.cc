...
struct OPTForCausalLM_output OPTForCausalLM::forward(const struct OPTForCausalLM_input &input){
    struct OPTForCausalLM_output output;
    ...
    Matrix3D<float> mock_output(logits_output, 1, 512, 768);
    read_to_array("sample_output.bin", logits_output, 512 * 768);

    Matrix3D<float> logits(logits_output, 1, 512, 50272);
    linear(mock_output, this->lm_head_weights, logits);
